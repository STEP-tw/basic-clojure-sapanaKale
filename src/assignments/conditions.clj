...
(defn duplicate-first
  ...
  {:level        :easy
   ...
   :implemented? true}
  [coll]
  (when-first [first coll] (conj (concat coll) first)))
...
(defn contains-in-order? [sub-coll coll]
  (= sub-coll (filter #(contains? (set sub-coll) %) coll)))
...
(defn zero-separated-palindrome
  ...
  {:level        :easy
   ...
   :implemented? true}
  [coll]
  (as-> (map inc coll) new-coll
        (concat (reverse new-coll) (conj new-coll 0))))
